...
export function bindImportEvents(handlers) {
    ...
    $overlay.on('click', '.lz-import-char-card', async function() {
        ...
        $('#lz-import-body').html(`<div style="text-align:center; padding:50px; opacity:0.6;"><i class="fa-solid fa-spinner fa-spin"></i> ${translate('Scanning Archives...', 'vistalyze.import.scanning')}</div>`);
        ...
        try {
            ...
            $('#lz-import-body').html(`<p style="text-align:center; padding:20px; color:var(--SmartThemeErrorColor);">${translate('Scan failed. See console.', 'vistalyze.import.scan_failed')}</p>`);
        }
    });
    ...
    $overlay.on('mouseenter', '.lz-import-all-btn', async function() {
        ...
        if (locations.length > 5) {
            ...
            tip += ` ${translate('and', 'vistalyze.import.and')} ${remaining} ${translate('more...', 'vistalyze.import.more')}`;
        }
        
        $(this).attr('title', `${translate('Imports:', 'vistalyze.import.imports_list')} ${tip}`);
    });
    ...
    async function resolveAndImport(importList) {
        ...
        if (window.toastr) window.toastr.success(translate('Imported {{count}} locations successfully.', 'vistalyze.import.success_count').replace('{{count}}', count), 'Vistalyze');
    }
